...
public class MySQLAdapter implements MySqlRepository {
    ...
    @Override
    public Mono<Branch> saveBranch(String branchName, String branchLocation) {
        BranchMySQL branchMySQLAux = new BranchMySQL();
        branchMySQLAux.setBranchName(branchName);
        branchMySQLAux.setBranchLocation(branchLocation);
        return branchRepository.save(branchMySQLAux).map(branchMySQL -> {
            return new Branch(BranchId.of(branchMySQL.getId().toString()),new BranchName(branchName),new BranchLocation(branchLocation)) ;
        });
